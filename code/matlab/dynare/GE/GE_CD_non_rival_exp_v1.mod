...
model;
    ...
    exp(k1) = ( sk * exp(k(-1)) ) ^ theta1 * exp(h(-1)) ^ (1 - theta1);
    
    // 5. Production function 2
    exp(y2)     = exp(k2) ^ alpha2 * (exp(z2) * exp(n2)) ^ gamma2;
    exp(k2) = ( (1 - sk ) * exp(k(-1)) ) ^ theta2 * exp(h(-1)) ^ (1 - theta2);
    ...
    //alpha1 * (1 - theta1) * exp(y1) / ( exp(h(-1)) )  = 
    //exp(q) * alpha2 * (1 - theta2) * exp(y2) / ( exp(h(-1)) );  
...
initval;
    ...
    k2 = log(( (1 - sk ) * exp(k) ) ^ theta2 * ( exp(h) ) ^ (1 - theta2));
    q  = log(0.539193);
