...
model;
    ...
    y1     = k1(-1) ^ alpha1 * (exp(z1) * l1) ^ gamma1;
    k1(-1) = ( theta1 * kT1(-1) ^ ((lambda1 - 1) / lambda1) + (1 - theta1) * kI1(-1) ^ ((lambda1 - 1) / lambda1) ) ^ (lambda1 / (lambda1 - 1));
    
    // 2. Production function 2
    y2     = k2(-1) ^ alpha2 * (exp(z2) * l2) ^ gamma2;
    k2(-1) = ( theta2 * kT2(-1) ^ ((lambda2 - 1) / lambda2) + (1 - theta2) * kI2(-1) ^ ((lambda2 - 1) / lambda2) ) ^ (lambda2 / (lambda2 - 1));
    
    // 3. Intangible capital accumulation
    kI     = y2 + (1 - deltaI) * kI(-1);

    // 4. First order equation for kT1
    alpha1 * theta1 * y1 / k1(-1) * (kT1(-1) / k1(-1)) ^ (- 1 / lambda1) = 
    mu * alpha2 * theta2 * y2 / k2(-1) * (kT2(-1) / k2(-1)) ^ (- 1 / lambda2);

    // 5. First order equation for kI1
    alpha1 * (1 - theta1) * y1 / k1(-1) * (kI1(-1) / k1(-1)) ^ (- 1 / lambda1) = 
    mu * alpha2 * (1 - theta2) * y2 / k2(-1) * (kI2(-1) / k2(-1)) ^ (- 1 / lambda2);    

    // 6. First order equation for kT
    (1 - deltaT) + mu(+1) * alpha2 * theta2 * y2(+1) / k2 * (kT2 / k2) ^ (- 1 / lambda2) =
    1 + ri;

    // 7. First order equation for kI
    mu(+1) * ( (1 - deltaI) + alpha2 * (1 - theta2) * y2(+1) / k2 * (kI2 / k2) ^ (- 1 / lambda2) ) = 
    (1 + ri) * mu;
